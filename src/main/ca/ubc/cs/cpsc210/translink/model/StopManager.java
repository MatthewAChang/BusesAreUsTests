...
public class StopManager implements Iterable<Stop> {
    ...
    public Stop getStopWithNumber(int number) {
        ...
        {
            Stop stop = new Stop(number, "", new LatLon(49.268335,-123.247816));
            this.stopMap.put(number, stop);
        }
    ...
    public void clearSelectedStop() {
        if(this.selected == null || !this.stopMap.containsValue(this.selected)) return;
        int num = 0;
        for(Stop stop: this)
            if(this.selected.equals(stop))
                num = stop.getNumber();

        this.stopMap.put(num, null);
        this.selected = null;
    }
    ...
    public Stop findNearestTo(LatLon pt) {
        ...
        for(Stop stop: this.stopMap.values())
        {
            if(SphericalGeometry.distanceBetween(pt, stop.getLocn()) < closest)
            {
                closest = SphericalGeometry.distanceBetween(pt, stop.getLocn());
                closestStop = stop;
            }
        }
        return closestStop;
