...
public class RouteMapParser {
    ...
    private void parseOnePattern(String str) {
        ...
        str = str.substring(1);
        String dashSplit[] = str.split(";")[0].split("-");
        String routeNumber = dashSplit[0];
        String patternName;
        if(dashSplit.length == 2)
            patternName = dashSplit[1];
        else
            patternName = dashSplit[1].concat("-").concat(dashSplit[2]);

        List<LatLon> elements = new ArrayList<>();
        String semiColonSplit[] = str.split(";");
        for(int i = 1; i < semiColonSplit.length; i += 2)
        {
            elements.add(new LatLon(Double.parseDouble(semiColonSplit[i]), (Double.parseDouble(semiColonSplit[i+1]))));
        }
